...
def q_train(make_obs_ph_n, act_space_n, q_index, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, scope="trainer", reuse=None, num_units=64):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        q = q_func(inputs=q_input, num_outputs=1, scope="q_func", num_units=num_units, num_layers=3)[:,0]
        ...
        # target network
        target_q = q_func(inputs=q_input, num_outputs=1, scope="target_q_func", num_units=num_units, num_layers=3)[:,0]
...
class MADDPGAgentTrainer(AgentTrainer):
    def __init__(self, name, model_q, model_p, obs_shape_n, act_space_n, agent_index, args, local_q_func=False):
        self.name = name
        self.n = len(obs_shape_n)
        self.agent_index = agent_index
        self.args = args
        #print(act_space_n[0].n)
        self.exploration_noise = OUNoise(act_space_n[0].n)
        obs_ph_n = []
        ...
        self.replay_buffer = ReplayBuffer(1e5)#1e6
    ...
    def noise_action(self, obs):

        return self.act(obs[None])[0]+self.exploration_noise.noise()

    def experience(self, obs, act, rew, new_obs, done, terminal):
        # Store transition in the replay buffer.
        # from IPython import embed; embed()
        self.replay_buffer.add(obs, act, rew, new_obs, np.array(done, dtype=np.float64))
    ...
    def update(self, agents, t):
        ...
        #    return
        if t < 500:  # only update every 100 steps
            return
