...
class EpisodesBufferEntry:
    ...
    def append(self, view, action, reward, done, probs=None):
...
class EpisodesBuffer(Buffer):
    ...
    def push(self, **kwargs):
        ...
        buffer = self.buffer
        #print('buffer:\n', buffer)
        index = np.random.permutation(len(view))  #生成随机序列
        ...
        for j in range(len(rewards)):
            ...
            if self.use_mean:
                entry.append(view[i], acts[i], rewards[i], dones[i], probs=probs[i])
            else:
                entry.append(view[i], acts[i], rewards[i], dones[i])
...
class AgentMemory(object):
    def __init__(self, obs_shape, act_n, max_len, use_mean=False):
        ...
        if self.use_mean:
            ...
            ##############mean_state
            self.me_state = MetaBuffer(obs_shape, max_len)

    def append(self, obs0, act, reward, dones, mean_sta=None, prob=None):
        self.obs0.append(np.array([obs0]))
        #self.feat0.append(np.array([feat0]))
        self.actions.append(np.array([act], dtype=np.int32))
        self.rewards.append(np.array([reward]))
        self.terminals.append(np.array([dones], dtype=np.bool))

        if self.use_mean:
            self.prob.append(np.array([prob]))
            ##############mean_state
            self.me_state.append(np.array([mean_sta]))
    ...
    def pull(self):
        res = {
           ...
           # 'feat0': self.feat0.pull(),
            ...
            'mean_sta': None if not self.use_mean else self.me_state.pull(),
...
class MemoryGroup(object):
    def __init__(self, obs_shape, act_n, max_len, batch_size, use_mean=False):
        self.agent = dict()
        self.max_len = max_len  #memory_size
        self.batch_size = batch_size
        self.obs_shape = obs_shape
        #self.feat_shape = feat_shape
        #self.sub_len = sub_len
        self.use_mean = use_mean
        self.act_n = act_n  #a_dim
    ...
    def _flush(self, **kwargs):
        ...
        if self.use_mean:
            ...
            ##############mean_state
            self.me_state.append(kwargs['mean_sta'])
    ...
    def push(self, **kwargs):
        for i, _ in enumerate(kwargs['state']):
            if self.agent.get(i) is None:
                self.agent[i] = AgentMemory(self.obs_shape, self.act_n, use_mean=self.use_mean, max_len=self.max_len)
            if self.use_mean:
                #self.agent[i].append(obs0=kwargs['state'][i], act=kwargs['acts'][i], reward=kwargs['rewards'][i], dones=kwargs['dones'][i], prob=kwargs['prob'][i])
                ##############mean_state
                self.agent[i].append(obs0=kwargs['state'][i], act=kwargs['acts'][i], reward=kwargs['rewards'][i], dones=kwargs['dones'][i], mean_sta=kwargs['mean_sta'][i], prob=kwargs['prob'][i])
            else:
                self.agent[i].append(obs0=kwargs['state'][i], act=kwargs['acts'][i], reward=kwargs['rewards'][i], dones=kwargs['dones'][i])
    ...
    def sample(self):
        ...
        if self.use_mean:
            ...
            me_state = self.me_state.sample(idx)
            me_state_next = self.me_state.sample(next_idx)
            return obs, me_state, actions, act_prob, obs_next, me_state_next, act_next_prob, rewards, dones, masks
        else:
...
class Runner(object):
    def __init__(self, sess, env, map_size, max_steps, model, play,
        ...
        if self.train:
            ...
            l_vars = self.model.vars  #获取神经网络变量
