...
class ValueNet:
    def __init__(self, nb_agent, a_dim, s_dim, obs_space, update_every=5, use_mf=False, learning_rate=1e-4, tau=0.99, gamma=0.95):
        ...
        with tf.variable_scope("ValueNet"):
            ...
            if self.use_mf:
                ...
                ##############mean_state
                self.mean_st = tf.placeholder(tf.float32, (None, self.s_dim), name='Mean-state')
    ...
    def _construct_net(self, trainable=True, reuse=False):
        ...
        if self.use_mf:
            ...
            ##############mean_state
            mean_st_emb = tf.layers.dense(self.mean_st, units=64, activation=tf.nn.relu)
            h_mean_st = tf.layers.dense(mean_st_emb, units=64, activation=tf.nn.relu)
            net = tf.concat([net, h_act_prob, h_mean_st], axis=1)
