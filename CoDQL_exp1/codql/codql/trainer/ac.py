...
class ActorCritic:
    def __init__(self, sess, name, a_dim, s_dim, obs_space, value_coef=0.1, ent_coef=0.08, gamma=0.95, batch_size=64, learning_rate=1e-4):
        self.sess = sess
        self.name = name
        self.view_space = obs_space
        ...
        with tf.variable_scope(name):
            ...
            self._create_network(self.view_space)
    ...
    def _create_network(self, view_space):
        ...
        h_view = tf.layers.dense(flatten_view, units=64, activation=tf.nn.relu)
        #h_emb = tf.layers.dense(input_feature,  units=256, activation=tf.nn.relu)
        #dense = tf.concat([h_view, h_emb], axis=1)
        dense2 = tf.layers.dense(h_view, units=64, activation=tf.nn.relu)
        dense3 = tf.layers.dense(dense2, units=64, activation=tf.nn.relu)
        dense4 = tf.layers.dense(dense3, units=64, activation=tf.nn.relu)
        dense = tf.layers.dense(dense4, units=64, activation=tf.nn.relu)
    ...
    '''def _create_network(self, view_space, feature_space):
        ...
        # fully connected
        flatten_view = tf.reshape(input_view, [-1, np.prod([v.value for v in input_view.shape[1:]])])
        h_view = tf.layers.dense(flatten_view, units=256, activation=tf.nn.relu)

        h_emb = tf.layers.dense(input_feature,  units=256, activation=tf.nn.relu)

        dense = tf.concat([h_view, h_emb], axis=1)
        dense = tf.layers.dense(dense, units=256 * 2, activation=tf.nn.relu)
    ...
    def train(self):
        ...
        for episode in batch_data:
            ...
            v, a, r = episode.views, episode.actions, episode.rewards
...
class MFAC:
    def __init__(self, sess, name, a_dim, s_dim, obs_space, value_coef=0.1, ent_coef=0.08, gamma=0.95, batch_size=64, learning_rate=1e-2):
        self.sess = sess
        self.name = name
    ...
    def _create_network(self, view_space):
        ...
        flatten_view = tf.reshape(input_view, [-1, np.prod([v.value for v in input_view.shape[1:]])])
        h_view = tf.layers.dense(flatten_view, units=64, activation=tf.nn.relu)
        dense = tf.layers.dense(h_view, units=64, activation=tf.nn.relu)
        ...
        dense_prob = tf.layers.dense(emb_prob, units=64, activation=tf.nn.relu)

        concat_layer = tf.concat([dense, dense_prob], axis=1)
        dense = tf.layers.dense(concat_layer, units=64, activation=tf.nn.relu)
        value = tf.layers.dense(dense, units=1)
        value = tf.reshape(value, (-1,))
        ...
        dense = tf.layers.dense(concat_layer, units=256 * 2, activation=tf.nn.relu)
        ...
        dense = tf.layers.dense(concat_layer, units=256, activation=tf.nn.relu)
        value = tf.layers.dense(dense, units=1)
        value = tf.reshape(value, (-1,))
