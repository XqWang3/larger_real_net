...
class TrafficEnv(gym.Env):
    ...
    def __init__(self, n=4, m=5, n_vih=100, add_newCar=5, max_keep_time=3, min_length=2, max_length=20,
                 flag_traffic_flow=1):
        '''
        ...
        self.max_keep_time = max_keep_time
    ...
    def reset(self):
        ...
        car_state_filename = 'step_{}.npz'.format(random.randint(3001, 4000))
        ...
        for j in range(n_vih_now):
            t = self.s_car[j][0]
            if t == 0:
                states[self.s_car[j][1]][self.s_car[j][2]] += 1
        for i in range(16):
            obs_n.append(self._get_obs(states, i))
    ...
    def updateLine(self, x1, y1, x2, y2):
        ...
        if x1 == x2:
            if y1 > y2:
                short_line.start = (x1 * 100 - 5 - n_vih1, y1 * 100 - m)
                short_line.end = (x1 * 100 - 5 + n_vih1, y1 * 100 - m)
            else:
                short_line.start = (x1 * 100 + 5 - n_vih1, y1 * 100 + m)
                short_line.end = (x1 * 100 + 5 + n_vih1, y1 * 100 + m)
        else:
            if x1 > x2:
                short_line.start = (x1 * 100 - m, y1 * 100 + 5 - n_vih1)
                short_line.end = (x1 * 100 - m, y1 * 100 + 5 + n_vih1)
            else:
                short_line.start = (x1 * 100 + m, y1 * 100 - 5 - n_vih1)
                short_line.end = (x1 * 100 + m, y1 * 100 - 5 + n_vih1)'''
    ...
    def step(self, a):
        ...
        delete_flag = [0 for _ in range(n_vih_now)]  # 准备删除的车辆 flag=1
        for i in range(n_vih_now):
            rest_nods = len(self.s_car[i]) - 2  # rest_nods=1时表示该车到达de下一路口即wei终点
            t = self.s_car[i][0]
            if t != 0:
                t -= 1
                self.s_car[i][0] = t
                self.decode_lines[self.s_car[i][1] // 4][self.s_car[i][1] % 4][self.s_car[i][2] // 4][
                    self.s_car[i][2] % 4][2] += 1
            elif rest_nods == 1:
                delete_flag[i] = 1  # 准备删除这个车辆的状态信息
            else:
                delete_s_flag = 0
                if self.s_light[self.s_car[i][2]][self.s_car[i][3]] == 1:  # 绿灯
                    ...
                    t = self.m
                    self.s_car[i][0] = t
                    self.decode_lines[self.s_car[i][2] // 4][self.s_car[i][2] % 4][self.s_car[i][3] // 4][
                        self.s_car[i][3] % 4][2] += 1
                else:
                    rewards[self.s_car[i][2]] -= 1  # 红灯，则此时该路口奖励-1
                    self.decode_lines[self.s_car[i][2] // 4][self.s_car[i][2] % 4][self.s_car[i][3] // 4][
                        self.s_car[i][3] % 4][1] += 1
        ...
        for i in range(16):
            ...
            for j in range(16):
                states[i].append(0)
        for k in range(n_vih_now):
            t = self.s_car[k][0]
            if t == 0:
                states[self.s_car[k][1]][self.s_car[k][2]] += 1
        for l in range(16):
            obs_n.append(self._get_obs(states, l))
        for i in range(16):
            rewards[i]=rewards[i]/(100*16)
        '''print('111111111')
        print(obs_n)
        print('========')
        print(rewards)
        print('222222222')
        print(n_vih_now)'''
        return obs_n, rewards, done_n, {}  # self.decode_lines
