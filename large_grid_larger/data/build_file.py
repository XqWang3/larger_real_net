...
def output_nodes(node):  #modified  #'  <node id="%s" x="%.2f" y="%.2f" type="%s"/>\n'
    str_nodes = '<nodes>\n'
    # traffic light nodes
    ind = 1
    for dy in np.arange(0, L0 * 7, L0): #L0 = 200
        for dx in np.arange(0, L0 * 7, L0):
            str_nodes += node % ('nt' + str(ind), dx, dy, 'traffic_light')
            ind += 1
    # other nodes
    ind = 1
    for dx in np.arange(0, L0 * 7, L0):
        str_nodes += node % ('np' + str(ind), dx, -L0_end, 'priority') #L0_end = 75
        ind += 1
    for dy in np.arange(0, L0 * 7, L0):
        str_nodes += node % ('np' + str(ind), L0 * 6 + L0_end, dy, 'priority')
        ind += 1
    for dx in np.arange(L0 * 6, -1, -L0):
        str_nodes += node % ('np' + str(ind), dx, L0 * 6 + L0_end, 'priority')
        ind += 1
    for dy in np.arange(L0 * 6, -1, -L0):
...
def output_edges(edge): #modified edge = '  <edge id="%s" from="%s" to="%s" type="%s"/>\n'
    str_edges = '<edges>\n'
    # external roads
    #in_edges = [5, 10, 15, 20, 25, 21, 16, 11, 6, 1]
    #out_edges = [6, 7, 8, 9, 10, 16, 17, 18, 19, 20]
    in_edges = [7, 14, 21, 28, 35, 42, 49, 43, 36, 29, 22, 15, 8, 1]
    out_edges = [8, 9, 10, 11, 12, 13, 14, 22, 23, 24, 25, 26, 27, 28]
    ...
    in_edges = [1, 2, 3, 4, 5, 6, 7, 49, 48, 47, 46, 45, 44, 43]
    out_edges = [1, 2, 3, 4, 5, 6, 7, 15, 16, 17, 18, 19, 20, 21]
    for in_i, out_i in zip(in_edges, out_edges):
        in_node = 'nt' + str(in_i)
        out_node = 'np' + str(out_i)
...
def output_connections(con): #modified '  <connection from="%s" to="%s" fromLane="%d" toLane="%d"/>\n'
    ...
    in_edges = [2, 3, 4, 5, 6, 48, 47, 46, 45, 44]
    out_edges = [2, 3, 4, 5, 6, 16, 17, 18, 19, 20]
    for i, j in zip(in_edges, out_edges):
        w_node = 'nt' + str(i - 1)
        e_node = 'nt' + str(i + 1)
        if i <= 7:
    ...
    for i in [9, 10, 11, 12, 13, 16, 17, 18, 19, 20, 23, 24, 25, 26, 27, 30, 31, 32, 33, 34, 37, 38, 39, 40, 41]:
        n_node = 'nt' + str(i + 7)
        s_node = 'nt' + str(i - 7)
        w_node = 'nt' + str(i - 1)
        e_node = 'nt' + str(i + 1)
...
def get_external_od(out_edges, dest=True): #modified
    #edge_maps = [0, 1, 2, 3, 4, 5, 5, 10, 15, 20, 25,
    #             25, 24, 23, 22, 21, 21, 16, 11, 6, 1]
    edge_maps = [0, 1, 2, 3, 4, 5, 6, 7, 7, 9, 21, 11, 35, 13, 49,
                 15, 48, 17, 46, 19, 44, 21, 43, 23, 29, 25, 15, 27, 1, 29]
    cur_dest = []
...
def init_routes(density): #0.2   modified
    ...
    in_nodes = [7, 14, 21, 28, 35, 42, 49, 43, 36, 29, 22, 15, 8, 1,
                1, 2, 3, 4, 5, 6, 7, 49, 48, 47, 46, 45, 44, 43]
    out_nodes = [8, 9, 10, 11, 12, 13, 14, 22, 23, 24, 25, 26, 27, 28,
    ...
    # streets
    k = 1
    car_num = int(MAX_CAR_NUM * density) #30*0.2=6
...
def output_flows(peak_flow1, peak_flow2, density, seed=None): #modified
    '''
    ...
    #srcs.append(get_external_od([16, 18, 20], dest=False))
    srcs.append(get_external_od([22, 24, 26, 28], dest=False))
    #srcs.append(get_external_od([2, 3, 4], dest=False))
    srcs.append(get_external_od([2, 4, 6], dest=False))
    #srcs.append(get_external_od([6, 8, 10], dest=False))
    srcs.append(get_external_od([8, 10, 12, 14], dest=False))

    sinks = []
    #sinks.append(get_external_od([2, 3, 4]))
    sinks.append(get_external_od([2, 4, 6]))
    #sinks.append(get_external_od([6, 8, 10]))
    sinks.append(get_external_od([8, 10, 12, 14]))
    #sinks.append(get_external_od([14, 13, 12]))
    sinks.append(get_external_od([20, 18, 16])) # go through
    #sinks.append(get_external_od([20, 18, 16]))
    sinks.append(get_external_od([28, 26, 24, 22])) # go through
    ...
    #ratios1 = np.array([0.4, 0.7, 1.0, 0.85, 0.6, 0.4, 0.25])  # start from 0
    #ratios2 = np.array([0.3, 0.8, 1.0, 0.8, 0.6, 0.4, 0.1])  # start from 15min
    #ratios1 = np.array([0.4, 0.7, 1.0, 0.75, 0.5, 0.25, 0.1])  # start from 0
    #ratios2 = np.array([0.3, 0.8, 1.0, 0.80, 0.6, 0.20, 0.1])  # start from 15min
    ...
    id1 = len(flows1) #7
    id2 = len(times) - 1 - id1 ##11-1-7=3
    for i in range(len(times) - 1): ##10
        name = str(i)
...
def output_ild(ild): #modified #'  <laneAreaDetector file="ild.out" freq="1" id="%s_%d" lane="%s_%d" pos="-50" endPos="-1"/>\n'
    str_adds = '<additional>\n'
    #in_edges = [5, 10, 15, 20, 25, 21, 16, 11, 6, 1,
    #            1, 2, 3, 4, 5, 25, 24, 23, 22, 21]
    #out_edges = [6, 7, 8, 9, 10, 16, 17, 18, 19, 20,
    ...
    for k, (i, j) in enumerate(zip(in_edges, out_edges)):
        ...
        str_adds += get_ild_str(node1, node2, ild) # added by wangxq
        #if k < 10:
        if k < 14:
            # streets
            str_adds += get_ild_str(node2, node1, ild, lane_i=1)
            str_adds += get_ild_str(node1, node2, ild, lane_i=1) # added by wangxq
    # streets
    #for i in range(1, 25, 5):
    for i in range(1, 49, 7):
        #for j in range(4):
...
def output_tls(tls, phase): #modified
    str_adds = '<additional>\n'
...
def main():
    ...
    write_file('./exp.tll.xml', output_tls(tls, phase))  #why three phases????????????? No why
