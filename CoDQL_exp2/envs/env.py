...
import pandas as pd
import subprocess
from envs.sumolib import checkBinary
import time
from envs import traci
...
class PhaseSet:
    def __init__(self, phases):
        self.num_phase = len(phases) #5
        self.num_lane = len(phases[0]) #12
        self.phases = phases
...
class TrafficSimulator:
    def __init__(self, config, output_path, is_record, record_stats, port=0):
        self.name = config.get('scenario') #large_grid
        ...
        self.port = DEFAULT_PORT + port #8000
        self.sim_thread = port  #0
        self.obj = config.get('objective') #hybrid
        self.data_path = config.get('data_path') #./large_grid/data/
        self.agent = config.get('agent')  #codql
        self.coop_gamma = config.getfloat('coop_gamma') #0.9
        self.cur_episode = 0
        self.norms = {'wave': config.getfloat('norm_wave'), #5.0
                      'wait': config.getfloat('norm_wait')} #100.0
        self.clips = {'wave': config.getfloat('clip_wave'), #2.0
                      'wait': config.getfloat('clip_wait')} #2.0
        ...
        self.init_test_seeds(test_seeds) ##10000,20000
        self._init_sim(self.seed) #12
        self._init_nodes()
        self.terminate()
    ...
    def _get_state(self):
        ...
        self._measure_state_step() #normalized state. node.wave_state=[lane0,lane1,..,lanen]; node.wait_state=[lane0,lane1,..,lanen]
    ...
    def _init_nodes(self):
        nodes = {}
        print('=============')
        print(self.sim.trafficlight.getIDList())
        print('=============')
        ...
        print('self.n_a_ls:', self.n_a_ls)
        self._init_state_space()
        print('self.n_s:', self.n_s)
        print('self.n_s_ls:', self.n_s_ls)
        print('self.n_f_ls:', self.n_f_ls)
        print('self.n_w_ls:', self.n_w_ls)
    ...
    def _init_sim(self, seed, gui=False): #12
        sumocfg_file = self._init_sim_config(seed)
        if gui:
            app = 'sumo-gui'
        else:
            app = 'sumo'
        command = [app, '-c', sumocfg_file]
    ...
    def _measure_reward_step(self):
        ...
        for node_name in self.node_names:
            ...
            if self.obj == 'queue':
                ...
                #print('node_wait:',wait)
                reward = - queue - self.coef_wait * wait   #coef_wait = 0.2
    ...
    def _measure_state_step(self):
        for node_name in self.node_names:
            node = self.nodes[node_name]
            for state_name in self.state_names: #[wave, wait]
                if state_name == 'wave':
                    cur_state = []
                ...
                norm_cur_state = self._norm_clip_state(cur_state,
                                                       self.norms[state_name], # wave:5, wait:100
                                                       self.clips[state_name]) ## wave:2, wait:2
                if state_name == 'wave':
    ...
    def init_test_seeds(self, test_seeds): #10000,20000
    ...
    def step(self, action):
        ...
        reward = self._measure_reward_step()  #raw reward
        done = False
        if self.cur_sec >= self.episode_length_sec: #3600
            done = True
        ...
        if self.agent in ['greedy']:
            reward = global_reward
        elif self.agent == 'codql':
            new_reward = []
            sum_rew = np.sum(reward)
            for i, rew in enumerate(reward):
                # begin calculate weight rew_n
                new_reward.append((sum_rew - rew) / len(self.n_s_ls) + rew)
            reward = np.array(new_reward)
        elif self.agent in ['dqn','ddpg']:
            reward = reward # each agent own particular reward
            ...
            if self.name == 'real_net':
                ...
                reward = reward / (len(self.node_names) * REALNET_REWARD_NORM) #REALNET_REWARD_NORM=20
        else: #self.agent == 'ma2c'
            ...
            for node_name, r in zip(self.node_names, reward):
                ...
                #    new_reward.append(cur_reward / (n_node * REALNET_REWARD_NORM)) ##REALNET_REWARD_NORM=20
            reward = np.array(new_reward)
        return state, reward, done, global_reward
