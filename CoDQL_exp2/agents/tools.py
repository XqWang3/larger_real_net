...
class AgentMemory(object):
    ...
    def append(self, obs0, act, reward, done, mean_sta=None, prob=None):
...
class MemoryGroup(object):
    def __init__(self, obs_shape, s_dim_wave, s_dim_wait, act_n, max_len, batch_size, use_mean=False):
        self.agent = dict()
        self.max_len = max_len  #memory_size
        self.batch_size = batch_size
        self.obs_shape = obs_shape
        #self.s_dim_wave = s_dim_wave
        #self.s_dim_wait = s_dim_wait
        self.use_mean = use_mean
    ...
    def push(self, **kwargs):
        for i, _ in enumerate(kwargs['state']):
            ...
            if self.use_mean:
                self.agent[i].append(obs0=kwargs['state'][i], act=kwargs['acts'][i], reward=kwargs['rewards'][i],
                                    done=kwargs['dones'][i], prob=kwargs['prob'][i])
                ##############mean_state
                #self.agent[i].append(obs0=kwargs['state'][i], act=kwargs['acts'][i], reward=kwargs['rewards'][i],
                #                     done=kwargs['dones'][i], mean_sta=kwargs['mean_sta'][i], prob=kwargs['prob'][i])
            else:
                self.agent[i].append(obs0=kwargs['state'][i], act=kwargs['acts'][i], reward=kwargs['rewards'][i], 
                                     done=kwargs['dones'][i])
